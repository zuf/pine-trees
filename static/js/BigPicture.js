...
(function() {
    ...
    function makeGallery(gallery) {
        if (Array.isArray(gallery)) {
            ...
            galleryEls = galleryEls.map(function(el) {
                var src = el.getAttribute("data-bp");
                return {
                    el: el,
                    src: src,
                    caption: el.getAttribute("data-caption"),
                    isVid: /\.(mp4|webm|ogg)/i.test(src)
                };
            });
        }
        // show loading icon / add to cache if needed
        // don't show for video bc we don't want to add to cache - this all needs to be refactored later
    ...
    function updateGallery(movement) {
        ...
        if (isEnd) {
            if (galleryEls[galleryPosition].error) {
                return;
            }
        ...
        [galleryPosition - 1, galleryPosition, galleryPosition + 1].forEach(
            function(position) {
                ...
                if (preloadedMedia[position]) return;
                var src = galleryEls[position].src;
                var mediaEl;
                // make vid el if video
                if (galleryEls[position].isVid) {
                    ...
                    mediaEl = doc[createEl]("IMG");
                    mediaEl[listenFor]("load", addToImgCache.bind(null, src));
                ...
                preloadedMedia[position] = mediaEl;
            }
        );

        // if media is loaded, show it
        if (preloadedMedia[galleryPosition].complete) {
        ...
        if (galleryEls[galleryPosition].isVid) {
            ...
            function checkGalleryVid(preloadVideo) {
                if (~[1, 4].indexOf(preloadVideo.readyState)) {
                    ...
                    timeout(checkGalleryVid, 35, preloadVideo);
                }
            }
            checkGalleryVid(preloadedMedia[galleryPosition]);
        } else {
            // is image
            preloadedMedia[galleryPosition].onload = function() {
                galleryOpen && changeGalleryImage(movement);
            };
        }
    }
    ...
    function changeGalleryImage(movement) {
        ...
        if (!activeEl.error) {
            ...
            if (!isVid) {
                displayImage = displayElement = preloadedMedia[galleryPosition];
            } else {
                displayVideo = displayElement = preloadedMedia[galleryPosition];
            }
            changeCSS(
                displayElement,
                "animation:" +
    ...
    function createIframe(isYoutube) {
        ...
        var url = isYoutube
            ? "www.youtube.com/embed/" + siteVidID + "?html5=1&rel=0&playsinline=1&"
    ...
    function removeContainer() {
        ...
        if (galleryOpen) {
            ...
            var currentDisplayEls = container.querySelectorAll("img, video");
            for (var i = 0; i < currentDisplayEls.length; i++) {
                container[removeEl](currentDisplayEls[i]);
            }
